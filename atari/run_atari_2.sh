...
# EXP_P=dmamba_pong
# for seed in 123 132 213 231 312 321; do python train_atari.py --game 'Pong' --data_dir_prefix $DATA_DIR --context_length 50 --token_mixer 'mamba' --output $OUT_DIR --experiment $EXP_P --seed $seed; done

# EXP_DTP=dtrans_pong
# for seed in 123 132 213 231 312 321; do python train_atari.py --game 'Pong' --data_dir_prefix $DATA_DIR --context_length 50 --token_mixer 'attn' --output $OUT_DIR --experiment $EXP_DTP --seed $seed; done
