...
for seed in 231 312; do python train_atari.py --game 'Breakout' --data_dir_prefix $DATA_DIR --context_length 5 --token_mixer 'mamba' --output $OUT_DIR --experiment $EXP_Q --seed $seed; done
...
for seed in 231 312; do python train_atari.py --game 'Breakout' --data_dir_prefix $DATA_DIR --context_length 5 --token_mixer 'attn' --output $OUT_DIR --experiment $EXP_DTQ --seed $seed; done